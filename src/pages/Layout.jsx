  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const userRole = user.app_role || 'agent';
        ...
        // If fetching user fails (e.g., not logged in), set a placeholder user with a 'guest' role or similar
        // to prevent infinite loading. For a real app, this would redirect to login or show public content.
        setCurrentUser({ app_role: 'guest' }); // Set a minimal user object to unblock rendering
        setNavigation([]); // No navigation for unauthenticated guests, or define a guest-specific navigation.
      }
    };
    fetchUser();
  }, []);
